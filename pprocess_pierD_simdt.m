...
model_set = ["20250306_2131"; "20250306_2132";...
    "20250408_1231"; "20250331_0753";...
    "20250408_1230"; "20250331_0825"];

mset_info = ["Locked_Rotor config - GMTO SIS Lat stiffness";...
    "Locked_Rotor config - IDOM SIS Lat stiffness";...
    "Stick_Tel/HBS config - GMTO SIS Lat stiffness";...
    "Stick_Tel/HBS config - IDOM SIS Lat stiffness";...
    "Stick_Tel/EDS config - GMTO SIS Lat stiffness";...
    "Stick_Tel/EDS config - IDOM SIS Lat stiffness"];

mset_info_red = ["LR-GMTO";"LR-IDOM";...
    "StickHBS-GMTO";"StickHBS-IDOM";"StickEDS-GMTO";"StickEDS-IDOM"];


sel_model_idx = 3:4;%1:2:6;
...
for i_ = 1:numel(model_set)
    dt_file = sprintf('./data/model-%s-RLE0%d_.parquet',model_set{i_},rle_id);
    ...
    label_struct = replace(extractBetween(dt_file,"data/model-","-RLE"),'_','\_');
...
fig1 = figure(200-rle_id);
set(gcf,'position',[923   350   740   400])
for ik = 1:3    
    subplot(3,1,ik)
...
pier_D = [zeros(12,2), reshape(cell2mat(sssha_data.Pier6D),12,[])];
